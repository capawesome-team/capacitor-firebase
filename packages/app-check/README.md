...
| **`debugToken`**                | <code>string</code>  | If debug token is set `true`, the predefined debug token can be used. If not set, a random debug token will be generated by Firebase SDK. Only available for Web. iOS and Android will use environment variables for this. Read more: - [Web](https://firebase.google.com/docs/app-check/web/debug-provider) - [iOS](https://firebase.google.com/docs/app-check/ios/debug-provider#ci) - [Android](https://firebase.google.com/docs/app-check/android/debug-provider#ci) | <code>false</code> | TBA   |
