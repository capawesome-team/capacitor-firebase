...
| **`stacktrace`**       | <code>StackFrame[]</code>          | A stacktrace generated by stacktrace.js. Note: On iOS this cannot be combined with `code`, `domain` and `customProperties`.                     | 1.1.0 |
| **`customProperties`** | <code>SetCustomKeyOptions[]</code> | Sets a custom key and value that is associated with exactly this report and not globally. Note: On iOS this cannot be combined with stacktrace. | 7.1.0 |
