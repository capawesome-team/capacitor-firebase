...
1. Add `facebook.com` to the `providers` [configuration](https://github.com/capawesome-team/capacitor-firebase/tree/main/packages/authentication#configuration) array.
1. If you are using **CocoaPods** for your iOS project, add the `CapawesomeCapacitorFirebaseAuthentication/Facebook` pod to your `Podfile` (usually `ios/App/Podfile`):

   ```diff
   target 'App' do
   capacitor_pods
   # Add your Pods here
   +  pod 'CapawesomeCapacitorFirebaseAuthentication/Facebook', :path => '../../node_modules/@capawesome/capacitor-firebase-authentication'
   end
   ```
